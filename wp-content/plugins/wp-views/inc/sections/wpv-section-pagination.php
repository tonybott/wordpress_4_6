...
class WPV_Editor_Pagination {
	...
	static function on_load() {
		...
		add_action( 'wp_ajax_wpv_update_archive_pagination',		array( 'WPV_Editor_Pagination', 'wpv_update_archive_pagination' ) );
		// Init properties
	...
	static function wpv_add_view_pagination( $view_settings, $view_id ) { //TODO review that default values are set before we display any of this
	...
	static function wpv_add_archive_pagination( $view_settings, $view_layout_settings, $view_id, $user_id ) {
		...
		$view_settings			= apply_filters( 'wpv_filter_wpv_get_pagination_defaults', $view_settings );
		$ajax_effects			= apply_filters( 'wpv_filter_wpv_pagination_ajax_effects', self::$pagination_ajax_effects, $view_id );
		$posts_per_page_options	= apply_filters( 'wpv_filter_extend_posts_per_page_options', self::$pagination_posts_per_page );
		...
		<div class="wpv-setting-container wpv-settings-pagination js-wpv-settings-pagination">
			...
			<div class="wpv-setting js-wpv-setting">
				<ul>
					<li>
						<input id="wpv-archive-pagination-type-disabled" class="js-wpv-archive-pagination-type" type="radio" name="wpv-archive-pagination-type" value="disabled" <?php checked( $view_settings['pagination']['type'] == 'disabled' ); ?> />
					...
					<li>
						<input id="wpv-archive-pagination-type-paged" class="js-wpv-archive-pagination-type" type="radio" name="wpv-archive-pagination-type" value="paged" <?php checked( $view_settings['pagination']['type'] == 'paged' ); ?> />
					...
					<li>
						<input id="wpv-archive-pagination-type-ajaxed" class="js-wpv-archive-pagination-type" type="radio" name="wpv-archive-pagination-type" value="ajaxed" <?php checked( $view_settings['pagination']['type'] == 'ajaxed' ); ?> />
				...
				<div class="wpv-advanced-setting js-wpv-archive-pagination-advanced-settings<?php if ( $view_settings['pagination']['type'] == 'disabled' ) { echo ' hidden'; } ?>">
					...
					<ul>
						<li>
							<?php 
							$select_structure = '<select class="js-wpv-archive-pagination-posts-per-page">'
	...
	static function wpv_update_archive_pagination() {
		if ( ! current_user_can( 'manage_options' ) ) {
			$data = array(
				'type' => 'capability',
				'message' => __( 'You do not have permissions for that.', 'wpv-views' )
		...
		$new_settings['effect']			= ( isset( $settings['effect'] ) && in_array( $settings['effect'], array( 'fade', 'slideh', 'slidev', 'infinite' ) ) ) ? esc_attr( $settings['effect'] ) : 'fade';
