...
class GoFetch_JR_Importer {
	...
	public static function set_feed_options( $feed, $url ) {
		...
		if ( empty( $_POST['_ajax_nonce'] ) || ! wp_verify_nonce( $_POST['_ajax_nonce'], 'goft_jobs_nonce' ) ) {
			return $feed;
		}

		if ( empty( $_REQUEST['action'] ) || 'goft_jobs_import_feed' !== $_REQUEST['action'] ) {
			return $feed;
		}
	...
	public static function import_feed( $url, $fetch_images = false, $cache = true ) {
		...
		if ( ! empty( $parsed_url['host'] ) ) {
			...
			$provider_id = 'unknown';
			$host        = __( 'Unknown', 'gofetch-jobs' );
		}

		$provider = GoFetch_JR_RSS_Providers::get_providers( $provider_id );
		...
		$valid_item_tags   = GoFetch_JR_RSS_Providers::valid_item_tags();
		$valid_regexp_tags = GoFetch_JR_RSS_Providers::valid_regexp_tags();
		...
		$vars = array(
			'context'  => 'STARTNG RSS FEED IMPORT PROCESS',
			'provider' => $provider,
		);
		...
		foreach( $items as $item ) {
			...
			if ( gofj_jr_fs()->is_plan__premium_only('pro') ) {
				...
				if ( ! empty( $provider['feed']['regexp_mappings'] ) ) {
					...
					$search_parts['title']       = $item->get_title();
					$search_parts['description'] = $item->get_description();
					...
					if ( ! empty( $provider['feed']['regexp_mappings']['title'] ) || ! empty( $provider['feed']['regexp_mappings']['description'] ) ) {
						...
						foreach( $provider['feed']['regexp_mappings'] as $tag => $regexp ) {
							...
							if ( $parts ) {
								...
								// As an exception, give priority to regexp matches in 'title' or 'description' over existing namespaces tags iteration (done below).
				...
				foreach( $namespaces as $attribute => $namespace ) {
					...
					foreach( $valid_item_tags as $tag ) {
						...
						if ( $data ) {
							...
							// Maybe log import info.
							$vars = array(
								'context'  => 'IMPORTING ITEM FROM RSS FEED (VALID NAMESPACES)',
								'item'     => $new_item['title'],
								'tag'      => $tag,
								'data'     => $data,
								'skipped?' => isset( $skip_tags[ $tag ] ) ? 'Yes (get from title or description)' : 'No',
							);
							bc_framework_simple_log( $vars, ( defined('WP_GOFT_DEBUG_LOG') && WP_GOFT_DEBUG_LOG ) );
				...
				if ( ! empty( $new_item['geolocation'] ) ) {
					...
					$new_item['longitude'] = -1 * abs( trim( $geolocation[1] ) );
				}
			...
			// Maybe log import info.
			$vars = array(
				'context'   => 'IMPORTING ITEM FROM RSS FEED',
				'new_item'  => $new_item,
			);
			bc_framework_simple_log( $vars, ( defined('WP_GOFT_DEBUG_LOG') && WP_GOFT_DEBUG_LOG ) );
	        ...
	        $new_items[] = apply_filters( 'goft_jobs_rss_item', $new_item );
		}
	...
	public static function import( $items, $params ) {
		...
		if ( empty( $items ) ) {
			trigger_error( __( 'No items found!', 'gofetch-jobs' ) );
		...
		self::$used_taxonomies = get_object_taxonomies( GoFetch_JR()->parent_post_type, 'objects' );

		$items = apply_filters( 'goft_jobs_import_items_before_filter', $items );

		$defaults = array(
			'post_type'       => GoFetch_JR()->parent_post_type,
			'post_author'     => 1,
			'tax_input'       => array(),
			'smart_tax_input' => '',
			'meta'            => array(),
		...
		$results = self::filter_items( $items, array( 'post_type' => $params['post_type'] ), $params['keywords'], $params['limit'] );

		extract( $results );

		$unique_items = apply_filters( 'goft_jobs_import_items_after_filter', $unique_items );
		...
		foreach( $unique_items as $item ) {
			...
			if ( ! apply_filters( 'goft_jobs_import_rss_item', true, $item, $params ) ) {
				continue;
			}
	...
	private static function _import( $item, $params ) {
		...
		do_action( 'goft_jobs_before_insert_job', $item, $params );
		...
		if ( $post_id = self::_insert_post( $item, $params ) ) {
			...
			do_action( 'goft_jobs_after_insert_job', $post_id, $item, $params );

			return true;
		}

		do_action( 'goft_jobs_insert_job_failed', $item, $params );
	...
	private static function _insert_post( $item, $params ) {
		...
		$post_content = apply_filters( 'goft_jobs_post_content', wp_kses_post( $item['description'] ), $item, $params );
		...
		if ( ! empty( $params['smart_tax_input'] ) ) {
			...
			if ( method_exists( 'GoFetch_JR_Premium_Pro_Features', 'smart_tax_terms_input' ) ) {
				$params['tax_input'] = GoFetch_JR_Premium_Pro_Features::smart_tax_terms_input( $params['tax_input'], $content, self::$used_taxonomies, $params['smart_tax_input'] );
			}
	...
	private static function _prepare_item_meta( $item, $meta, $post_id, $params ) {
		...
		foreach( $meta as $meta_key => $meta_value ) {
			...
			if ( ! apply_filters( 'goft_jobs_item_skip_meta', false, $meta_value, $meta_key, $item, $post_id, $params ) ) {
				$meta[ $meta_key ] = apply_filters( 'goft_jobs_item_meta_value', $meta_value, $meta_key, $item, $post_id, $params );
			}
		}
		return $meta;
	}
	 ...
	 */
	private static function filter_items( $items, $args = array(), $keywords = '', $limit = 0 ) {

		$defaults = array(
			'post_type'   => 'post',
			'post_status' => 'any',
			'meta_key'    => '_goft_jobs_is_external',
			'nopaging'    => true,
		);
		$args = apply_filters( 'goft_jobs_filter_items_query_args', wp_parse_args( $args, $defaults ) );
		...
		foreach( $items as $item ) {
			...
			if ( empty( $external_posts ) || ! isset( $external_posts[ $item['link'] ] ) ) {
				...
				if ( $keywords && $content ) {
					...
					if ( ! GoFetch_JR_Helper::match_keywords( $content, $keywords ) ) {
						$excluded_items[] = $item;
						continue;
					} else {
						$unique_items[] = $item;
