jQuery(document).ready(function($) {
	...
	$('#templates_list').select2({
		placeholder: goft_jobs_admin_l18n.label_templates,
	});
	...
	$('#providers_list').select2({
		...
		placeholder             : goft_jobs_admin_l18n.label_providers,
	...
	$(document).on( 'guided_tour.init', function( e, data ) {
		...
		$(document).on( 'click', '.button.gofetch-jobs-tour1_0_step4_2', function() {
	...
	$(document).on( 'change', '#goft-settings-type', function(e) {
		...
		var data = {
			action      : 'goft_jobs_toggle_settings',
			toggle      : toggle,
			_ajax_nonce : goft_jobs_admin_l18n.ajax_nonce,
		};

		$.post( goft_jobs_admin_l18n.ajaxurl, data );
	...
	$(document).on( 'click', '.refresh-templates', function(e) {
		...
		if ( ! $('.goft_jobs.processing.templates').length ) {

			template_actions.hide().after('<div class="goft_jobs processing templates">&nbsp;</div>');

			$('.import-feed').prop( 'disabled', true );
		}

		var data = {
			action: 'goft_jobs_update_templates_list',
			_ajax_nonce: goft_jobs_admin_l18n.ajax_nonce,
		};

		$.post( goft_jobs_admin_l18n.ajaxurl, data, function( response ) {
			...
			$('.goft_jobs.processing.templates').remove();
	...
	$(document).on( 'click', '.import-feed', function(e) {
		...
		if ( ! url ) {
			alert( goft_jobs_admin_l18n.msg_specify_valid_url );
			e.preventDefault();
			return;
		}
		...
		$('.goft_jobs.no-jobs-found').remove();

		if ( ! $('.goft_jobs.processing.feed').length ) {
			$('input[name=import_feed]').hide();
			$('input[name=import_feed]').after('<span class="goft_jobs processing feed">&nbsp;</span>').after('<a class="button-secondary cancel-feed-load">' + goft_jobs_admin_l18n.cancel_feed_load + '</a>');
			$('input', template_actions).prop( 'disabled', true );
		}

		var data = {
			action      : 'goft_jobs_import_feed',
			url         : url,
		...
		xhr = $.post( goft_jobs_admin_l18n.ajaxurl, data, function( response ) {
			...
			if ( undefined !== response ) {
				...
				if ( undefined !== response.error ) {
					...
					$('input[name=load_images]').closest('p').after('<span class="goft_jobs no-jobs-found feed"> ' + goft_jobs_admin_l18n.msg_invalid_feed + ': ' + response.error );

				} else if ( response.total_items > 0 ) {

					$('.goft_jobs_jobs_found').remove();

					$('.goft_jobs_table').html( response.sample_item );

					$('.goft_jobs_table').after( '<h4 class="goft_jobs_jobs_found code">' + goft_jobs_admin_l18n.msg_jobs_found + ': ' + response.total_items + '</h4>');
					...
					$('input[name=load_images]').closest('p').after('<span class="goft_jobs no-jobs-found feed"> ' + goft_jobs_admin_l18n.msg_no_jobs_found + '</span>');
				}

			}

			$('.goft_jobs.processing').remove();
	...
	$(document).on( 'click', '.save-template', function(e) {
		...
		if ( ! template_name ) {
			alert( goft_jobs_admin_l18n.msg_template_missing );
			return;
		}

		if ( ! $('.goft_jobs.processing.save').length ) {
			$('.save-template').hide();
			$('.save-template').after('<span class="goft_jobs processing save">&nbsp;</span>');
		}

		var data = {
			action           : 'goft_jobs_save_template',
			...
			_wpnonce         : goft_jobs_admin_l18n.ajax_save_nonce,
		};
		...
		$.post( goft_jobs_admin_l18n.ajaxurl, data, function( response ) {
			...
			if ( undefined !== response && '1' === response) {
				...
				$('.save-template').show();
				$('.save-template').after('<span class="template-saved-msg"> ' + goft_jobs_admin_l18n.msg_template_saved + '</span>');
			...
			$('.goft_jobs.processing.save').remove();
	...
	$(document).on( 'change', '#templates_list', function() {
		...
		var data = {
			action: 'goft_jobs_load_template_content',
			template: $(this).val(),
			_ajax_nonce: goft_jobs_admin_l18n.ajax_nonce,
		};
	...
	$(document).on( 'change', '#providers_list', function() {
		...
		$('.goft_jobs.processing.providers').remove();
		...
		if ( ! $('.goft_jobs.processing.templates').length ) {
			$('#providers_list').next('.select2').after('<div class="goft_jobs processing providers">&nbsp;</div>');
		}

		var data = {
			action      : 'goft_jobs_load_provider_info',
		...
		$.post( goft_jobs_admin_l18n.ajaxurl, data, function( response ) {
			...
			if ( response ) {
				...
				$('.providers-placeholder-content').append( '<a class="dashicons-before dashicons-dismiss close-provider" title="' + goft_jobs_admin_l18n.title_close + '"></a>' );
	...
	$(document).on( 'click', 'a.provider-rss, a.provider-rss-custom', function(e) {
		...
		$(this).after('<span class="rss-copied-msg"> ' + goft_jobs_admin_l18n.msg_rss_copied + '</span>').fadeIn();
	...
	$('.section-expand').on( 'click', function(e) {
		...
		$( context ).closest('tr').toggle( function() {
			...
			if ( $(this).is(':visible') ) {
				$( section_a ).text( goft_jobs_admin_l18n.simple );
			} else {
				$( section_a ).text( goft_jobs_admin_l18n.advanced );
			}
	...
	function auto_fill_custom_fields_defaults( provider ) {
		...
		if ( undefined !== goft_jobs_admin_l18n.default_query_args ) {
			all_query_args.push( goft_jobs_admin_l18n.default_query_args );
		}
		...
		$( context ).each( function() {
			...
			$( 'select, input[type!="button"], textarea', tr ).each( function() {
				...
				if ( $(this).is('select') ) {
					...
					value = $(this).is(':checked') ? goft_jobs_admin_l18n.label_yes : goft_jobs_admin_l18n.label_no;
				} else if ( $(this).val() ) {
					value = $(this).val();
				...
				if ( ! $( '.' + selector ).length ) {
					...
					var sel_values_placeholder =
						'<tr class="section-advanced temp-tr-hide">' +
							'<td><a href="#" class="section-expand" expand="' + section + '">' + goft_jobs_admin_l18n.msg_advanced + '</a></td>' +
							'<td class="tip">&nbsp;</td>' +
	...
	function single_provider_item ( state ) {
		...
		if ( ! state.id ) {
			...
			if ( state.children ) {
				var $state = $('<span class="goft-jobs-group">' + state.text + '<span class="dashicons-before ' + icon + ' expand-minimize"></span></span>');
				return $state;
			}
	...
	$('#_goft_jobs_period_custom').closest('tr').hide();

	$(document).on( 'change', '#_goft_jobs_period', function() {

		if ( 'custom' == $(this).val() ) {
			$('#_goft_jobs_period_custom' ).closest('tr').fadeIn();
		} else {
			$('#_goft_jobs_period_custom' ).closest('tr').hide();
		}
	...
	$('#_goft_jobs_period').trigger('change');
