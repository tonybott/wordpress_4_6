 ...
 */
class GoFetch_JR_Sample_Table extends GoFetch_JR_HTML_Table {
	...
	public static function display( $query_args = array(), $fields = array() ) {
		...
		// output the table
		$table = new GoFetch_JR_Sample_Table( $fields, $query_args['content_type'] );
	...
	public function show() {
		echo $this->table( $this->fields, array( 'class' => 'goft_jobs_table widefat fields' ) );
	}
	...
	public function header( $data ) {
		...
		$cells = $this->cells( array(
			__( 'Fields', 'gofetch-jobs' ),
			__( 'Sample Content', 'gofetch-jobs' ),
		), $atts, 'td' );
	...
	protected function row( $item = array(), $atts = array() ) {
		...
		if ( empty( $item['data'] ) ) {
			...
			$item['data'] = __( '[serialized data]', 'gofetch-jobs' );
		}
	...
	private function make_fields_struct( $fields, $type = 'custom', $content_type = 'post' ) {
		...
		foreach( $fields as $field => $data ) {
			...
			$default = array(
				'name'              => $field,
				'type'              => $type,
				'content_data_type' => GoFetch_JR_Helper::get_field_type( $field, $content_type ),
				'data'              => '-',
				'label'             => '',
				'checked'           => false,
			);
